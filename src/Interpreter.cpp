...
Interpreter::Interpreter(std::unordered_map<std::string, std::string>& vars,
...
void Interpreter::parseLine(const std::string& line) {
    ...
    // Check for array declaration
        ...
        while (std::getline(ss, item, ',')) {
            item.erase(std::remove_if(item.begin(), item.end(), ::isspace), item.end()); // Trim whitespace
...
void Interpreter::executeConsoleLog(const std::string& arg) {
    ...
    if (variables.find(processedArg) != variables.end()) {
        ...
        std::cout << processedArg << std::endl; // Print the argument directly if it's not a variable or array
    }
}

void Interpreter::declareVariable(const std::string& name, const std::string& value) {
    variables[name] = value;
}

void Interpreter::declareArray(const std::string& name, const std::vector<std::string>& values) {
    arrays[name] = values;
}